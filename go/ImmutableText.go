...
type Node interface {
	Length() int
	SubNode(int, int) Node
	GetCharAt(int) rune
	CopyTo (int, []rune, int, int)
}

type WideLeafNode struct {
	data []rune
}

func (this WideLeafNode) Length() int {
	return len(this.data)
}

func (this WideLeafNode) SubNode(start int, end int) Node {
	if start == 0 && end == this.Length() {
		return this
	}
	var subArray = make ([]rune, end - start);
	copy(subArray, this.data[start:end])
	return WideLeafNode { subArray }
}

func (this WideLeafNode) GetCharAt(index int) rune {
